...
- **Doctor**: reads `mercury.env` itself (works on a host whose service is the thing being diagnosed). healthz check against `http://127.0.0.1:${MERCURY_PORT:-3000}/healthz`; one smoke Run per enabled harness via `POST /api/runs` (needs `MERCURY_ADMIN_TOKEN` or `MERCURY_API_TOKENS`; without one the smoke section says so instead of crashing — and since #648 the all-skipped case fails the doctor). Since #654 an explicit empty `MERCURY_HARNESSES` also fails the doctor ("nothing to smoke-verify") unless `--allow-no-harnesses` says the host runs none; an unset variable still means the default list. The first build's Fleet-reachability check was removed with the pull model (issue #645). Since #665 a second healthz verifies the address Fleet will actually dial when `MERCURY_BIND_HOST` is a named address; since #674 a `0.0.0.0` bind is not skipped — the host's own first non-internal IPv4 (`os.networkInterfaces()`) is dialed and reported (`healthz (bind 0.0.0.0, dialed <ip>): PASS/FAIL`), with an explicit `SKIP — no non-internal IPv4 …` note on a loopback-only host. Every check is bounded; `--json` emits the same facts as the human report (`noHarnesses` since #654).
- Tests: `test/hostService.test.ts` (9) + `test/hostDoctor.test.ts` (28) — deterministic units, path resolution, dry-run writes nothing, no-env failure, unknown subcommand/flag, healthz/Fleet/smoke against a mock HTTP server, env-file parsing, the #665/#674 bind-address second check (injectable interface lists; no LAN socket in tests).
