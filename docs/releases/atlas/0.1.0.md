...
> **Not published yet.** `0.1.0` is not on the npm registry, there is no `atlas-v0.1.0` tag, and
> there is no GitHub Release for it. This file ships in the release PR so the `atlas-v0.1.0` job
> finds its release body here, and the release notes gain the install command only when the
> version is actually on the registry — [§15](../../knowledge-base.md) of the design forbids
> advertising an install that does not exist, and `test/releaseDocs.test.ts` holds these notes to
> that rule against the live registry.
>
