...
its own, so there is no `cli-v0.1.0` and nothing to install separately -- installing the host RC
installs the RC client. `mercuryctl --version` prints `mercuryctl 0.1.0`.
