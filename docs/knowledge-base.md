...
- `.mercury-context.json` is written per Run by the PrimeAgent, RPC, daemon and Claude Code
  adapters and describes the task, not the project.
...
The rules below read `tool.*` and `test.*`, so they have **no input on a backend whose
`capabilities.static.toolEvents` is `'none'`** -- Hermes today (#594), because quiet mode exposes no
machine-readable event stream to distil from. That is a missing input, not a Run that produced
nothing worth recording, and a consumer that cannot tell those apart will report a harness as
unproductive because its transcript is blind.
...
The `.mercury-context.json` that the PrimeAgent, RPC, daemon and Claude Code adapters write gains a
...
| `ClaudeCodeAdapter` | `CLAUDE.md` in the workspace root, **only if none is tracked** in the repository; otherwise a line in the stdin task text naming `.mercury/knowledge/NOTES.md`; plus the `.mercury-context.json` pointer in the task text on every Run | Claude Code reads `CLAUDE.md` from the working directory; a tracked one is the project's and must not be edited by Mercury (§9.4). The fallback names `NOTES.md` rather than the context pointer this row originally specified, because §9.2 originally said the context file is written by the PrimeAgent, RPC and daemon adapters -- **not** by this one -- so pointing at it here would have sent the model to a file no Claude Run had. The adapter now writes the context file like the other three (issue #612), so the pointer names a file that exists; whether Claude Code reads it unprompted is unverified, which is why the task text carries the pointer rather than trusting discovery. |
| `HermesAgentAdapter` | `AGENTS.md` in the workspace root, **only if none is tracked** in the repository; otherwise the neutral files only (§9.4 forbids editing a tracked file) | Measured: Hermes v0.21.2 reads `AGENTS.md` from the working directory unprompted and does **not** read `.mercury/knowledge/NOTES.md` (#541, one Run each, observed once). `persona.append` remains the design for the persona channel, but it is not needed for this: `AGENTS.md` is the channel that exists. |
...
One honest caveat remains on the adoption half: the note's claim was false for these fixtures
(`hello-obs` has no Makefile rule), so what is measured is trust-and-attempt, not successful
command adoption; the `primeagent` row's note named a command that existed. The write-back
half is now measured too (2026-09-22 stack, widened 2026-09-26): the scoped
`MERCURY_CLAUDE_ALLOWED_TOOLS` allowlist gained `Write`/`Edit` on `.mercury/notes.jsonl` --
scoped to the one file, verified first that the same task without those rules stays
permission-blocked -- and both treated Runs closed the loop end to end: the agent appended a
well-formed line, the finalize harvest validated it (`knowledge.noted`), the outbox pushed it,
and Atlas holds it as a `candidate` (`note_749eeec958277813` from the pointer-channel Run
`run_87afd91b01204687`, `note_a9a4123e3e3c30c1` from the generated-channel Run
`run_0f5a28c4d3d24619`, both `source: agent-reported`, `claude` 2.1.260). A first treated Run
(`run_297154f0c1e94471`) wrote the file but its line was refused `invalid-contradicts` because
it carried `contradicts` as a string instead of an array of note ids -- the write-back loop
works, and the validator still polices the shape. Both candidates contradict the promoted
bait note; they stay candidates until an operator promotes them.
...
5. **Tier 3 and the remaining adapters.** Decision-record harvesting and the operator index
   ...
   Run read the pointed-at file first -- §10 carries the detail). The `claude`
   note write-back closed the same way on 2026-09-26: with `Write`/`Edit` on
   `.mercury/notes.jsonl` scoped into the observation stack's allowlist, the treated pair wrote
   tier-1 notes that survived validation and reached Atlas as candidates
   (`note_749eeec958277813`, `note_a9a4123e3e3c30c1`; §10 carries the Run ids), so both halves
   of the loop -- read and write -- are now observed on `claude`.
