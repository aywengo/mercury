...
- Stashing with `git stash -u` in a worktree whose `node_modules` is an untracked symlink -- the normal
  way to avoid a second `npm ci`. `-u` stashes the symlink, the next run has no dependencies, and ~34
  failures appear across `api.test.ts`, `auth.test.ts` and `fleetContract.test.ts` that look like a
  catastrophic regression and are an empty `node_modules`. Use a separate worktree to get a clean tree
  (issue #596, where this cost a full invalid baseline).
